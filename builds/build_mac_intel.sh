...
echo "=========================================================="
echo "    Building PDF Document Explorer for Intel Macs"
...
echo "🔨 Building Intel macOS application with Semantic Search..."
echo "🎯 Target: Intel x86_64 Architecture"
...
if command -v codesign &> /dev/null; then
    ...
    if [ ! -z "$SIGNING_IDENTITY" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            codesign --verify --verbose "dist/PDF Document Explorer Intel.app"
            if [ $? -eq 0 ]; then
...
echo "📁 The application is located in: dist/PDF Document Explorer Intel.app"
echo "🧠 Features: PDF viewing + AI-powered semantic search"
echo "🍎 Target: macOS 10.15+ (Catalina) for Intel Macs"
echo "🔧 Architecture: Built for x86_64 (Intel processors)"
echo ""
echo "🎉 You can now distribute this .app bundle to Intel Mac users!"
echo ""
