...
class PDFExtractor:
    ...
    def __init__(self, pdf_path: str, output_path: str = "pdf_content.json", use_pdfplumber: bool = False):
    ...
    def extract_pdf_pages_stream(self) -> Iterator[Tuple[int, str]]:
        ...
        self.logger.info(f"Streaming content from {self.pdf_path} using {'pdfplumber' if self.use_pdfplumber else 'PyPDF2'}")
        
        if self.use_pdfplumber:
            yield from self._extract_with_pdfplumber()
        else:
            yield from self._extract_with_pypdf2()
    
    def _extract_with_pdfplumber(self) -> Iterator[Tuple[int, str]]:
    ...
    def _extract_with_pypdf2(self) -> Iterator[Tuple[int, str]]:
...
def main():
    ...
    parser.add_argument('--use-pdfplumber', action='store_true',
                       help='Use pdfplumber instead of PyPDF2 for extraction')
    
    args = parser.parse_args()
    
    # Extract the document
    extractor = PDFExtractor(args.pdf_path, args.output, args.use_pdfplumber)
